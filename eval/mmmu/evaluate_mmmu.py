...
class MMMUDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, split, prompt, input_size=224, dynamic_image_size=False,
        ...
        for subject in tqdm(CAT_SHORT2LONG.values()):
            sub_dataset = load_dataset(root, subject, split=split, cache_dir="/resource/mmu_local")
...
def evaluate_chat_model():
    if args.o1_pred:
        prompt = {
            'multiple-choice': "Think step-by-step first and then answer with the option's letter from the given choices directly.",
            'open': 'Think step-by-step first and then answer the question with a single number.'
        }
    else:
        prompt = {
