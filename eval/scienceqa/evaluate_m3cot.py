...
ds_collections = {
    ...
    'm3cot_test': {
        'root': '/dataset/LLaVA-NeXt/eval/m3cot/val_eval.json',
        'max_new_tokens': 100,
        'min_new_tokens': 1,
    },
}
...
class ScienceQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = "/dataset/LLaVA-NeXt/eval/m3cot/images/" + image_path
...
def evaluate_chat_model():
    prompt = "Think step-by-step first and then answer with the option's letter from the given choices directly." if args.o1_pred else "Answer with the option's letter from the given choices directly."
...
if __name__ == '__main__':
    ...
    print(os.getenv('WORLD_SIZE'), os.getenv('RANK'))
    assert args.batch_size == 1, 'Only batch size 1 is supported'
    #assert int(os.getenv('WORLD_SIZE')) == 1, "error"
