...
ds_collections = {
    ...
    'chartqa_test_human': {
        'train': '/dataset/InternVL/chartqa/train_human.jsonl',
        'test': '/dataset/InternVL/chartqa/test_human.jsonl',
        'metric': 'relaxed_accuracy',
        'max_new_tokens': 100,
    },
    'chartqa_test_augmented': {
        'train': '/dataset/InternVL/chartqa/train_augmented.jsonl',
        'test': '/dataset/InternVL/chartqa/test_augmented.jsonl',
        'metric': 'relaxed_accuracy',
        'max_new_tokens': 100,
    },
    'gqa_testdev': {
    ...
    'ai2diagram_test': {
        ...
        'test': '/dataset/InternVL/ai2diagram/test_vlmevalkit.jsonl',
        'metric': 'accuracy',
        'max_new_tokens': 10,
    },
    'infographicsvqa_val': {
...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #image = "/dataset/LLaVA-NeXt/eval/textvqa" + image        # custom image root
        ...
        if "Answer with the option's letter from the given choices directly." in question:
            question = question.split("\nAnswer with the option's letter")[0]
    ...
    for ds_name in args.datasets:
        ...
        if o1_mode:
            if 'textvqa' in ds_name:
                input_prompt = "\nThink step-by-step first and then answer the question with a single number, word, or phrase."
            elif 'ai2d' in ds_name:
                input_prompt = "\nThink step-by-step first and then answer with the option's letter from the given choices directly."
        ...
        if o1_mode:
            skip_special = False
            ds_collections[ds_name]['max_new_tokens'] += 4000
        else:
            skip_special = True
