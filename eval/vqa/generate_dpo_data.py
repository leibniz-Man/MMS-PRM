...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = "dataset/clean_image_root/" + image        # custom image root
...
def evaluate_chat_model(o1_mode=False):
    ...
    for ds_name in args.datasets:
        ...
        outputs = []
        for _, (pixel_values, questions, question_ids, images, answers) in tqdm(
...
if __name__ == '__main__':
    ...
    # Select proper backend/device for CUDA/NPU/CPU
    ...
    # Prefer to register HCCL backend when on Ascend
    try:
        import torch_npu  # noqa: F401
    except Exception:
        pass
    # Silence harmless NCCL default-timeout warning on non-NCCL builds
    ...
    device = torch.device('npu' if backend == 'hccl' else 'cuda' if backend == 'nccl' else 'cpu')
