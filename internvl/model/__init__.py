...
def split_model(num_layers, vit_alpha=1.0):
    device_map = {}
    # Support both CUDA and Ascend NPU
    try:
    ...
    # Fallback for single device to avoid division by zero when (world_size - vit_alpha) == 0
    if world_size <= 1:
        # Put everything on device 0
...
def load_model_and_tokenizer(args):
    ...
    if not args.load_in_8bit and not args.load_in_4bit and not args.auto:
        # Move model to available accelerator
        if hasattr(torch, 'cuda') and torch.cuda.is_available():
            model = model.cuda()
        elif hasattr(torch, 'npu') and torch.npu.is_available():
            model = model.to('npu')
