...
import transformers
try:
    from decord import VideoReader  # optional: used for fast video decoding
    _DECORD_AVAILABLE = True
except Exception:
    VideoReader = None
    _DECORD_AVAILABLE = False
    print('Warning: decord is not installed or unavailable. Falling back to OpenCV/ImageIO for video decoding when needed.')
...
def read_frames_opencv(
        video_path, num_frames, sample='rand', fix_start=None,
        client=None, clip=None, min_num_frames=4
):
    # Support both local path and s3 path (by writing a temporary file)
    temp_file = None
    video_path_local = video_path
    if 's3://' in video_path and client is not None:
        import tempfile
        temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.mp4')
        temp_file.write(client.get(video_path))
        temp_file.flush()
        video_path_local = temp_file.name
    ...
    if clip:
        ...
        vlen = max(0, min(vlen, end_index) - start_index)
        ...
        if image_type == 'image':
            ...
            if fn.endswith('/'):
                ...
                if _DECORD_AVAILABLE:
                    ...
                    frames = read_frames_opencv(fn, num_frames=max_num_frames, min_num_frames=min_num_frames,
                                                client=self.client, sample=sample, clip=clip)
            return frames
