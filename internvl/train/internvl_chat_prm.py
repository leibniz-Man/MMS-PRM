...
from internvl.model.internvl_chat.modeling_internvl_prm import InternVLChatModel
...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        label_sources = [deepcopy(data_item['conversations'])]

        sources[0][1]["value"] = sources[0][1]["value"].split("<|split_token|>")[0]
        label_sources[0][1]["value"] = label_sources[0][1]["value"].split("<|split_token|>")[1]

        # Preprocess the conversations and generate the return dictionary
        ret = preprocess_function(self.template_name, sources,
                                  self.tokenizer, [self.num_image_token * num_patches],
                                  group_by_length=self.group_by_length,
                                  use_packed_ds=self.use_packed_ds, ds_name=self.ds_name)
        
        ret_label = preprocess_function(self.template_name, label_sources,
                                  self.tokenizer, [self.num_image_token * num_patches],
                                  group_by_length=self.group_by_length,
                                  use_packed_ds=self.use_packed_ds, ds_name=self.ds_name)
...
def main():
    ...
    # Auto-resolve backend to support CUDA (nccl) and Ascend NPU (hccl)
    init_dist(launcher=launcher, backend='auto')
