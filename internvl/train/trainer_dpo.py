...
class MultimodalDPOTrainer(DPOTrainer):
    def __init__(self, *args, **kwargs):
        ...
        # Force-disable on-policy generation during training.
        # Our pipeline provides precomputed chosen/rejected pairs,
        # so TRL should not call model.generate inside the training loop.
        # This guards against shell scripts accidentally enabling it
        # and against TRL versions that default to generation.
        try:
            if getattr(self.args, "generate_during_training", None):
                self.args.generate_during_training = False
        except Exception:
            # Be robust if args object differs across TRL versions
            pass
